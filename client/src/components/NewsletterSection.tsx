  ...
  return (
    <section ref={sectionRef} className="section-navy" style={{ padding: "6rem 0" }}>
      <div className="container">
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "4rem", alignItems: "center" }} className="newsletter-grid">
          ...
          <div className="reveal">
            ...
            <div style={{ display: "flex", gap: "0.75rem", flexWrap: "wrap" }}>
              <button className="btn-primary" onClick={onOpenModal} style={{ fontSize: "0.9375rem" }}>
                <Mail size={16} /> Join The Gym Growth Newsletter
              </button>
            </div>

            <div style={{ display: "flex", gap: "1.5rem", marginTop: "1.5rem", flexWrap: "nowrap", alignItems: "center" }}>
              {["Free forever", "Weekly drops"].map((item) => (
                <div key={item} style={{ display: "flex", alignItems: "center", gap: "0.375rem" }}>
                  ...
                  <span style={{ fontSize: "0.875rem", color: "rgba(255,255,255,0.6)", whiteSpace: "nowrap" }}>{item}</span>
                </div>
              ))}
            </div>
          </div>
          ...
          <div className="stagger" style={{ display: "flex", flexDirection: "column", gap: "0.875rem" }}>
            {snippets.map((s) => (
              <div
                key={s.title}
              className="lift-card-on-blue"
              style={{ padding: "1.125rem 1.375rem", display: "flex", alignItems: "center", gap: "1rem", cursor: "pointer" }}
                onClick={onOpenModal}
              >
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  <div style={{ fontSize: "0.9rem", fontWeight: 600, color: "#F0F0F5", lineHeight: 1.4, marginBottom: "0.25rem" }}>{s.title}</div>
                  <div style={{ fontSize: "0.75rem", color: "#7070A0" }}>{s.read}</div>
                </div>
                <ArrowRight size={15} color="#7070A0" style={{ flexShrink: 0 }} />
              </div>
            ))}
          </div>
        </div>
      ...
      <style>{`
        @media (max-width: 768px) { .newsletter-grid { grid-template-columns: 1fr !important; } }
      `}</style>
    </section>
  );
}
